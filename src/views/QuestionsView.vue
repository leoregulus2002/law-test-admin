...
const createForm = reactive({ number: 1, stem: '', questionType: 'SINGLE_CHOICE' as QuestionType, options: [] as Option[], answers: [] as string[], analysis: '' })
...
const typeLabels: Record<QuestionType, string> = { SINGLE_CHOICE: '单选题', MULTIPLE_CHOICE: '多选题', INDETERMINATE_CHOICE: '不定项', SUBJECTIVE: '主观题' }
...
function defaultOptions(): Option[] { return ['A', 'B', 'C', 'D'].map(label => ({ label, content: '' })) }
function resetCreateForm() { Object.assign(createForm, { number: 1, stem: '', questionType: 'SINGLE_CHOICE', options: defaultOptions(), answers: ['A'], analysis: '' }) }
function openCreate() {
  if (!banks.value.length) { error.value = '请先在题库管理中创建题库'; return }
  createBankId.value = filters.questionBankId || String(banks.value[0].id)
  resetCreateForm()
  createOpen.value = true
}
function closeCreate() { createOpen.value = false }
async function createQuestion() {
  if (!createBankId.value) { error.value = '请选择所属题库'; return }
  creating.value = true
  try {
    const payload = createForm.questionType === 'SUBJECTIVE' ? { ...createForm, options: [], answers: [] } : createForm
    await api.post(`/api/v1/question-banks/${createBankId.value}/questions`, payload)
    filters.questionBankId = createBankId.value
    closeCreate()
    search()
  } catch (reason: any) { error.value = reason.response?.data?.detail ?? reason.response?.data?.message ?? '新增题目失败，请检查题目内容' }
...
async function save() {
  ...
  saving.value = true
  try {
    const payload = form.questionType === 'SUBJECTIVE' ? { ...form, options: [], answers: [] } : form
    const { data } = await api.put<QuestionDetail>(`/api/v1/questions/${selected.value.id}`, payload)
...
<template>
  <AppShell>
    ...
    <div class="page-heading"><div><h1>题目管理</h1><p>集中查看、维护和控制各题库中的题目。</p></div><button class="button primary" @click="openCreate">新增题目</button></div>
    <form class="question-filter-bar" @submit.prevent="search"><div class="search-box"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6"/><path d="m16 16 4 4"/></svg><input v-model.trim="filters.keyword" placeholder="搜索题目标题" /></div><select v-model="filters.questionType" aria-label="答案类型"><option value="">全部答案类型</option><option value="SINGLE_CHOICE">单选题</option><option value="MULTIPLE_CHOICE">多选题</option><option value="INDETERMINATE_CHOICE">不定项</option><option value="SUBJECTIVE">主观题</option></select><select v-model="filters.status" aria-label="启用状态"><option value="">全部状态</option><option value="ACTIVE">已启用</option><option value="DISABLED">已停用</option></select><select v-model="filters.questionBankId" aria-label="所属题库"><option value="">全部题库</option><option v-for="bank in banks" :key="bank.id" :value="String(bank.id)">{{ bank.name }}</option></select><button class="button primary" type="submit">查询</button><button class="button secondary" type="button" @click="resetFilters">重置</button></form>
    ...
    <div v-if="detailOpen" class="modal-backdrop" @click.self="closeDetail"><section class="question-detail-modal" role="dialog" aria-modal="true" aria-labelledby="question-detail-title"><header class="import-modal-header"><div><span class="eyebrow">题目详情</span><h2 id="question-detail-title">{{ selected ? `第 ${selected.number} 题` : '正在加载' }}</h2><p v-if="selected">{{ selected.questionBankName }}</p></div><button class="icon-button" aria-label="关闭题目详情" @click="closeDetail">×</button></header><div v-if="detailLoading" class="detail-loading">正在加载题目内容…</div><form v-else-if="selected" class="question-detail-content" @submit.prevent="save"><template v-if="editing"><div class="edit-grid"><label>题号<input v-model.number="form.number" min="1" type="number" required /></label><label>题目类型<select v-model="form.questionType"><option value="SINGLE_CHOICE">单选题</option><option value="MULTIPLE_CHOICE">多选题</option><option value="INDETERMINATE_CHOICE">不定项</option><option value="SUBJECTIVE">主观题</option></select></label></div><label class="detail-field">题目标题<textarea v-model.trim="form.stem" rows="3" required /></label><div v-if="form.questionType !== 'SUBJECTIVE'" class="detail-field"><b>选项与答案</b><div v-for="option in form.options" :key="option.label" class="edit-option"><label><input v-model="form.answers" :value="option.label" type="checkbox" /><span>{{ option.label }}</span></label><input v-model.trim="option.content" :aria-label="`${option.label} 选项内容`" required /></div></div><label class="detail-field">{{ form.questionType === 'SUBJECTIVE' ? '参考答案' : '解析' }}<textarea v-model.trim="form.analysis" rows="4" /></label></template><template v-else><div class="detail-meta"><span class="type-pill">{{ typeLabels[selected.questionType] }}</span><span :class="['status-pill', selected.status === 'ACTIVE' ? 'active' : 'disabled']"><i />{{ selected.status === 'ACTIVE' ? '已启用' : '已停用' }}</span></div><h3>{{ selected.stem }}</h3><div v-if="selected.questionType !== 'SUBJECTIVE'" class="option-list"><div v-for="option in selected.options" :key="option.label" :class="{ answer: selected.answers.includes(option.label) }"><b>{{ option.label }}</b><span>{{ option.content }}</span><i v-if="selected.answers.includes(option.label)">正确答案</i></div></div><div v-if="selected.analysis" class="analysis-box"><b>{{ selected.questionType === 'SUBJECTIVE' ? '参考答案' : '题目解析' }}</b><p>{{ selected.analysis }}</p></div></template><footer class="question-detail-footer"><template v-if="editing"><button class="button secondary" type="button" :disabled="saving" @click="cancelEditing">取消</button><button class="button primary" :disabled="saving">{{ saving ? '保存中…' : '保存修改' }}</button></template><template v-else><button class="text-danger" type="button" :disabled="saving" @click="remove">删除</button><span class="footer-spacer" /><button class="button secondary" type="button" :disabled="saving" @click="toggleStatus">{{ selected.status === 'ACTIVE' ? '停用题目' : '启用题目' }}</button><button class="button primary" type="button" @click="startEditing">修改题目</button></template></footer></form></section></div>
    <div v-if="createOpen" class="modal-backdrop" @click.self="closeCreate"><section class="question-detail-modal" role="dialog" aria-modal="true" aria-labelledby="create-question-title"><header class="import-modal-header"><div><span class="eyebrow">手动录入</span><h2 id="create-question-title">新增题目</h2><p>选择题库并填写题干、选项和答案。</p></div><button class="icon-button" aria-label="关闭新增题目弹窗" @click="closeCreate">×</button></header><form class="question-detail-content" @submit.prevent="createQuestion"><div class="edit-grid"><label>所属题库<select v-model="createBankId" required><option v-for="bank in banks" :key="bank.id" :value="String(bank.id)">{{ bank.name }}</option></select></label><label>题号<input v-model.number="createForm.number" min="1" type="number" required /></label><label>题目类型<select v-model="createForm.questionType"><option value="SINGLE_CHOICE">单选题</option><option value="MULTIPLE_CHOICE">多选题</option><option value="INDETERMINATE_CHOICE">不定项</option><option value="SUBJECTIVE">主观题</option></select></label></div><label class="detail-field">题目标题<textarea v-model.trim="createForm.stem" rows="3" required /></label><div v-if="createForm.questionType !== 'SUBJECTIVE'" class="detail-field"><b>选项与答案</b><div v-for="option in createForm.options" :key="option.label" class="edit-option"><label><input v-model="createForm.answers" :value="option.label" type="checkbox" /><span>{{ option.label }}</span></label><input v-model.trim="option.content" :aria-label="`${option.label} 选项内容`" required /></div></div><label class="detail-field">{{ createForm.questionType === 'SUBJECTIVE' ? '参考答案' : '解析' }}<textarea v-model.trim="createForm.analysis" rows="4" /></label><footer class="question-detail-footer"><button class="button secondary" type="button" :disabled="creating" @click="closeCreate">取消</button><button class="button primary" :disabled="creating">{{ creating ? '保存中…' : '保存题目' }}</button></footer></form></section></div>
  </AppShell>
</template>
